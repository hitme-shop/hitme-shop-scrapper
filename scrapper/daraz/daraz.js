...
class darazScrapper{
   ...
   async queryCategories ( selector, c){
      ...
      let cats = []; this.$(selector).each((__, _) => {
         if (c.remove) { this.$(_).find(c.child).remove() }
         let name = this.$(_).find('a > span').text()
         let url = 'https:' + this.$(_).find('a').attr('href')
         cats.push({ name, url })
      }); return cats
   }
   ...
   async getSubCategories() {
      return await this.queryCategories('.lzd-site-menu-sub-item',
         { remove: true, child: '.lzd-site-menu-grand' })
   }

   async getCategories() {
      return await this.queryCategories('.lzd-site-menu-grand-item', { remove: false })
   }
   ...
   async getSliders() {
      return await this.page.$$eval('.card-banner-slider-main-span a', a => a.map(b => {
         let src = "https:"+ (b.firstElementChild.getAttribute('src') || b.firstElementChild.getAttribute('data-ks-lazyload'))
   ...
   async getFlashSells() {
      let deals = [];
      this.$('.flash-sale-body > div:first-child > .item-list-content > a')
         .each(async (__, _) => {
            let url = "https:" + this.$(_).attr('href')
            let title = this.$(_).find('.sale-title').text()
            let tags = title.toLowerCase().split(" ")
            let srcElement = this.$(_).find('img.image')
            let src = srcElement.attr('src') || srcElement.attr('data-ks-lazyload')
            let sPrice = formatTk(this.$(_).find('.sale-price').text())
            let oPrice = formatTk(this.$(_).find('.origin-price-value').text())
            let discount = calcDiscount(oPrice, sPrice)
            let rating = 0; let ratingCount = 0
            let flag = 'flashSale'
            //let availability = this.$(_).find('.pg-text').text().toUpperCase()
            deals.push({
               title, tags,url, src, sPrice, oPrice, discount,
               rating,ratingCount,flag, website:this.website
            })
         }); return deals
   }
   ...
   async getJustForYou() {
      ...
      $('.card-jfy-li-content').each((__, _) => {
         ...
         let title = $(_).find('.card-jfy-title > span.title').text()
         let tags = title.toLowerCase().split(" ")
         ...
         products.push({
            title, tags ,url, src, sPrice, oPrice, discount, rating, ratingCount,website:this.website, flag
         })
      })
      return products
   }
   ...
   async getProducts(page){
      ...
      return await page.evaluate(async () => {
         ...
         for (pDiv of proDivs) {
            ...
            let sPrice = text('span.c13VH6').replace(/৳|,/g, '').trim() * 1
            let oPriceEl = pDiv.querySelector('.c3lr34 .c1-B2V')
            let oPrice = 0;oPriceEl !== null || undefined ?oPrice = oPriceEl.innerText.replace(/৳|,/g, '').trim() * 1 :oPrice = sPrice 
            ...
            let src = '';
            try { src = getImage() } catch (error) {
               await revealImage(pDiv.querySelector('.cRjKsc'), 1500)
               src = getImage()
            }
            products.push({
               title, src, url,sPrice, oPrice, discount, rating,
               ratingCount, website: "Daraz", flag:"Category"
            })
         }
         return products
      })
